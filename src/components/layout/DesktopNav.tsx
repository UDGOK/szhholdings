...
const DesktopNav = ({ isScrolled, navItems, investmentItems }: DesktopNavProps) => {
  ...
  return (
    <div className="hidden md:flex items-center space-x-14 px-4 py-2 rounded-full bg-black/10 backdrop-blur-sm">
      {navItems.map((item) => (
        item.hasDropdown ? (
          <div
            key={item.name}
          ...
          >
            <Tooltip content={item.tooltip}>
              <button
                className={cn(
                  'text-[15px] font-normal tracking-wide transition-all duration-200 focus:outline-none group-hover:scale-105 relative',
                  isScrolled
                    ? 'text-gray-900 hover:text-primary'
                    : 'text-white hover:text-white/80',
                  'after:content-[""] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-white after:scale-x-0 after:origin-left after:transition-transform hover:after:scale-x-100'
                )}
              >
                {item.name}
              </button>
            ...
            <AnimatePresence>
              {isInvestmentsOpen && (
                <motion.div
                  ...
                  className="absolute left-0 mt-1 w-52 bg-gray-900/95 backdrop-blur-sm shadow-lg border border-gray-700/50 rounded-lg py-1.5 z-50"
                >
                  {investmentItems.map((subitem) => (
                    <Tooltip key={subitem.name} content={subitem.tooltip} position="right">
                      <Link
                        href={subitem.href}
                        className="relative block px-5 py-2.5 text-[14px] text-gray-200 hover:text-white transition-colors overflow-hidden group"
                      >
                        ...
                        <span className="absolute inset-0 bg-gray-700/50 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300" />
                      </Link>
                    </Tooltip>
                  ))}
                </motion.div>
